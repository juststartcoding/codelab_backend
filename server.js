...
io.on("connection", (socket) => {
  ...
  socket.on("terminal:run", ({ roomId, code, language, stdin }) => {
    ...
    io.to(roomId).emit("terminal:output", {
      text: "\x1b[90m\u25b6 Running...\x1b[0m\n",
      type: "system",
    });

    const tmpDir = fsTerm.mkdtempSync(
      pathTerm.join(osTerm.tmpdir(), "cl-term-"),
    );
    ...
    try {
      switch (language) {
        case "c":
          ...
          compileCmd = [
            "gcc",
            [filename, "-o", pathTerm.join(tmpDir, "main"), "-lm"],
          ];
        ...
        case "cpp":
          ...
          compileCmd = [
            "g++",
            [filename, "-o", pathTerm.join(tmpDir, "main"), "-std=c++17"],
          ];
          runArgs = [pathTerm.join(tmpDir, "main"), []];
          break;
        case "python":
      ...
      if (compileCmd) {
        try {
          execSyncTerm(`${compileCmd[0]} ${compileCmd[1].join(" ")}`, {
            timeout: 8000,
            stdio: "pipe",
          });
        } catch (e) {
          ...
          try {
            fsTerm.rmSync(tmpDir, { recursive: true, force: true });
          } catch {}
          return;
        }
      }
      ...
      // Run
      const proc = spawn(runArgs[0], runArgs[1], {
  ...
  socket.on(
    "tutor:run-output",
    ({ roomId, output, error, status, language }) => {
      const room = activeRooms.get(roomId);
      if (!room) return;
