...
router.get("/my", auth, requireRole("tutor"), async (req, res) => {
  try {
    const assignments = await Assignment.find({ tutorId: req.user._id })
      .sort({ createdAt: -1 })
      .lean();
...
router.get("/available", auth, requireRole("student"), async (req, res) => {
  try {
    ...
    const subMap = {};
    subs.forEach((s) => {
      if (!subMap[s.assignmentId]) subMap[s.assignmentId] = s;
    });
    res.json(
      assignments.map((a) => ({ ...a, mySubmission: subMap[a._id] || null })),
    );
  } catch (e) {
    res.status(500).json({ message: e.message });
  }
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    if (req.user.role === "student") {
      a.testCases = a.testCases.map((t) =>
        t.isHidden
          ? { ...t, expectedOutput: "[hidden]", input: "[hidden]" }
          : t,
      );
    }
    res.json(a);
...
router.post("/:id/submit", auth, requireRole("student"), async (req, res) => {
  try {
    ...
    for (const tc of assignment.testCases) {
      ...
      let ok = false;
      const matchType = tc.matchType || "exact";
      if (matchType === "exact") {
        ok = actualOutput === expectedOutput;
      } else if (matchType === "contains") {
        ok = actualOutput.toLowerCase().includes(expectedOutput.toLowerCase());
      } else if (matchType === "regex") {
        try {
          ok = new RegExp(expectedOutput, "i").test(actualOutput);
        } catch {
          ok = false;
        }
      }
