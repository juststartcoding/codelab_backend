...
async function populateStudent(project) {
  ...
  const student = project.studentId ? await db.users.findOne({ _id: project.studentId }) : null;
  return { ...project, studentId: student ? { _id: student._id, name: student.name, email: student.email } : null };
}
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'student' && project.studentId !== req.user._id)
...
router.post('/', auth, requireRole('student'), async (req, res) => {
  try {
    ...
    const ext = lang === 'cpp' ? 'cpp' : lang === 'python' ? 'py' : lang === 'java' ? 'java' : lang === 'javascript' ? 'js' : 'c';
    const project = await db.projects.insert({
      name, description: description || '',
      studentId: req.user._id,
      language: lang,
      files: [{ _id: makeId(), name: `main.${ext}`, language: lang, content: getDefaultContent(lang), createdAt: new Date(), updatedAt: new Date() }],
      comments: [],
      isSubmitted: false,
      grade: '',
      tutorFeedback: '',
...
router.post('/:id/files', auth, async (req, res) => {
  try {
    ...
    const newFile = { _id: makeId(), name, language: language || 'c', content: getDefaultContent(language || 'c'), createdAt: new Date(), updatedAt: new Date() };
    const files = [...project.files, newFile];
    await db.projects.update({ _id: req.params.id }, { $set: { files, updatedAt: new Date() } });
  ...
  try {
    ...
    const files = project.files.map(f =>
      f._id === req.params.fileId
        ? { ...f, ...(req.body.content !== undefined && { content: req.body.content }), ...(req.body.name && { name: req.body.name }), updatedAt: new Date() }
        : f
    );
    await db.projects.update({ _id: req.params.id }, { $set: { files, lastEditedBy: req.user._id, updatedAt: new Date() } });
    const updated = await db.projects.findOne({ _id: req.params.id });
    res.json(updated);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
// Delete file
router.delete('/:id/files/:fileId', auth, requireRole('student'), async (req, res) => {
  try {
    const project = await db.projects.findOne({ _id: req.params.id });
    if (!project) return res.status(404).json({ message: 'Project not found' });
    if (project.studentId !== req.user._id) return res.status(403).json({ message: 'Access denied' });
...
// Tutor: add comment
router.post('/:id/comments', auth, requireRole('tutor', 'admin'), async (req, res) => {
  try {
    const project = await db.projects.findOne({ _id: req.params.id });
    if (!project) return res.status(404).json({ message: 'Project not found' });
    const comment = { _id: makeId(), tutorId: req.user._id, tutorName: req.user.name, ...req.body, createdAt: new Date() };
    const comments = [...project.comments, comment];
    await db.projects.update({ _id: req.params.id }, { $set: { comments, updatedAt: new Date() } });
    const updated = await db.projects.findOne({ _id: req.params.id });
...
router.patch('/:id/feedback', auth, requireRole('tutor', 'admin'), async (req, res) => {
  try {
    await db.projects.update({ _id: req.params.id }, { $set: { grade: req.body.grade || '', tutorFeedback: req.body.feedback || '', updatedAt: new Date() } });
    const updated = await db.projects.findOne({ _id: req.params.id });
    res.json(await populateStudent(updated));
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.post('/run', auth, async (req, res) => {
  ...
  if (!code) return res.status(400).json({ output: 'No code provided', error: true });
  ...
  try {
    ...
    switch (language) {
      ...
      case 'javascript':
        ...
        break;
      default:
        return res.json({ output: `Language "${language}" is not supported for server-side execution.`, error: false });
    }
    ...
    // Run
    try {
      const stdout = execSync(runCmd, { timeout, stdio: 'pipe', maxBuffer: 1024 * 1024 }).toString();
      res.json({ output: stdout || '(no output)', error: false, status: 'Accepted' });
    } catch (runErr) {
      const msg = runErr.stdout?.toString() || runErr.stderr?.toString() || runErr.message;
    ...
    try { fs_run.rmSync(tmpDir, { recursive: true, force: true }); } catch {}
  }
});
