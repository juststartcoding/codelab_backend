...
router.get('/active', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'student') {
      // Get student batches
      const myBatches = await db.batches?.find({ studentIds: req.user._id }) || [];
      const myBatchIds = myBatches.map(b => b._id);
      // Show session if: public (no batchId) OR student is in that batch
      allActive = allActive.filter(s => !s.batchId || myBatchIds.includes(s.batchId));
    }

    const result = allActive.map(s => ({ ...s, tutorId: { _id: s.tutorId, name: s.tutorName } }));
    res.json(result);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.get('/room/:roomId', auth, async (req, res) => {
  try {
    ...
    res.json({ ...session, tutorId: { _id: session.tutorId, name: session.tutorName } });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.patch('/:id/end', auth, requireRole('tutor'), async (req, res) => {
  try {
    await db.sessions.update(
      ...
      { $set: { isActive: false, endedAt: new Date(), codeSnapshot: req.body.codeSnapshot || '' } }
    );
    const session = await db.sessions.findOne({ _id: req.params.id });
    if (!session) return res.status(404).json({ message: 'Session not found' });
    res.json(session);
