...
const db = require('../db');
const { signToken, hashPassword, comparePassword, stripPassword } = require('../helpers');
...
router.post('/student/register', async (req, res) => {
  try {
    ...
    const user = await db.users.insert({ name, email: email.toLowerCase(), password: hashed, role: 'student', isApproved: true, createdAt: new Date(), lastLogin: null });
...
async function doLogin(req, res, role) {
  try {
    ...
    if (role === 'tutor' && !user.isApproved) return res.status(403).json({ message: 'Account pending admin approval' });
...
router.get('/me', auth, (req, res) => res.json({ user: stripPassword(req.user) }));
